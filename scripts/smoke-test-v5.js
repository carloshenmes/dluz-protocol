...
async function main() {
  ...
  console.log("╔═══════════════════════════════════════════════════╗");
  console.log("║   dLuz Protocol — Smoke Test v5                   ║");
  console.log("╚═══════════════════════════════════════════════════╝");
  ...
  const tx1 = await sale.buy({ value: parse("0.001") });
  await tx1.wait();
  const dluzAfter = await dluz.balanceOf(signer.address);
  ...
  const tx2 = await bridge.deposit(c.MockBCT, parse("50"));
  await tx2.wait();
  const dcAfter = await dcarbon.balanceOf(signer.address);
  const dcDelta = dcAfter - dcBefore;
  ...
  const tx3 = await registry.retire(parse("10"), "Smoke test v5 - carbon offset");
  await tx3.wait();
  const deAfter = await denergy.balanceOf(signer.address);
  const deDelta = deAfter - deBefore;
  ...
  const tx4a = await farming.stake(0, stakeAmt);
  const stakeReceipt = await tx4a.wait();
  console.log(`  Staked 100 DLUZ at block ${stakeReceipt.blockNumber}`);
  check("Farming: staked 100 DLUZ", true);

  // 4b: Wait — each Base Sepolia block is ~2s, need several blocks for rewards
  console.log("  ⏳ Waiting 20s for rewards...");
  await new Promise(r => setTimeout(r, 20000));

  // Send a dummy tx to force a new block with fresh timestamp
  const dummyTx = await signer.sendTransaction({ to: signer.address, value: 0n });
  await dummyTx.wait();
  ...
  const tx4b = await farming.claim(0);
  const claimReceipt = await tx4b.wait();
  const dcAfterClaim = await dcarbon.balanceOf(signer.address);
  const claimDelta = dcAfterClaim - dcBeforeClaim;
  console.log(`  Claimed: ${fmt(claimDelta)} dCARBON (block ${claimReceipt.blockNumber})`);
  check("Farming: claim dCARBON rewards", claimDelta > 0n);

  // 4d: Unstake
  const dluzBeforeUnstake = await dluz.balanceOf(signer.address);
  const tx4c = await farming.unstake(0, stakeAmt);
  await tx4c.wait();
  const dluzAfterUnstake = await dluz.balanceOf(signer.address);
  const unstakeDelta = dluzAfterUnstake - dluzBeforeUnstake;
